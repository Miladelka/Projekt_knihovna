-- Tento příkaz by měl dát výsledek všech řádků s nejmenším počtem dohromady, ale nejde spustit 
...
WHERE v.Id_No IN (SELECT Id_No FROM rezervace_fixed)
    ...
    AND NOT EXISTS (
        ...
        FROM
            rezervace_fixed
        WHERE
            admin_serial = r.admin_serial
            AND r.Id_No = Id_No
            AND date_request > r.date_request
            AND date_request < v.date_borrow)
GROUP BY
    r.admin_serial
ORDER BY
    DayDifference DESC